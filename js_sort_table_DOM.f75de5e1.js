...
(function() {
    ...
    if (!tbody) return;
    const headerTextAt = (colIdx)=>thead?.querySelectorAll('th')[colIdx]?.textContent.trim().toLowerCase() ?? '';
    const getTypeForColumn = (colIdx)=>{
        const headerName = headerTextAt(colIdx);
        return headerName === 'age' || headerName === 'salary' ? 'number' : 'string';
    };
    const getCellText = (tr, colIdx)=>tr.children[colIdx]?.textContent.trim() ?? '';
    ...
    const sortByColumn = (colIdx)=>{
        ...
        rows.sort((a, b)=>{
            ...
            let cmp;
            if (type === 'number') cmp = toNumber(avRaw) - toNumber(bvRaw);
            else cmp = avRaw.localeCompare(bvRaw, undefined, {
                sensitivity: 'base'
            });
    ...
    if (tfoot) tfoot.addEventListener('click', onHeaderClick);
})();
